...
We take the worst result for \texttt{433.milc} for discussion. For an exploration threshold value of one, we merge 58 functions for this
benchmark. Through profiling, we discovered that a handful of them are hot, that is they are frequently executed. If we prevent these hot
...
5.11\% code size reduction and an 18\% performance overhead. By avoiding merging hot functions, we can obtain little performance impact and
a code size reduction of 2.09\%. While almost 9$\times$ less, this is still almost twice as good as the state-of-the art. As with the
