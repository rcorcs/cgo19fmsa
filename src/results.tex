...
\begin{figure*}[t!]
  ...
  \includegraphics[width=\linewidth]{figs/reduction-results.pdf}
  %\vspace{-2ex}
...
All optimizations are implemented in LLVM v8 and evaluated on the C/C++ SPEC CPU2006~\cite{spec} benchmark suite. We target two different
...
Our function-merging technique is the \textit{first} technique able to
merge these functions, as we overcome major limitations from the existing
techniques.
...
Figure~\ref{fig:code-size-reduction-mibench} shows that 
...
the number of instructions at the IR level.

\textbf{talk about the benchmarks that have no effect}
