...
Some of the programs for these devices can have binaries of several megabytes in size,
with ever-increasing complexity.
For these programs, memory size can be a showstopper~\cite{plaza18}.
In these constrained scenarios, code-size optimizations are essential~\cite{schultz03,varma04,sehgal12,keoh14,auler17}.
...
and MSVC~\cite{msvc-icf}, implement variants of this optimization.
While they are only capable of merging identical functions~\cite{tallam10,kwan12,livska14},
the state-of-the-art~\cite{edler14} is able to also merge non-identical functions
by leveraging structural similarity.
