...
in size, turning memory size into a bottleneck~\cite{plaza18}. Just adding more
...
While sounding too simple to work, it is crucial for making high-level
abstractions usable, even when they introduce duplicate code~\cite{tallam10,kwan12}.
...
More advanced research approaches~\cite{edler14} have extended this idea into
...
While an improvement, even this state-of-the-art often fails to produce any
...
are not due to the lack of duplicate code but due to the rigid, ad-hoc 
...
a ranking-based exploration mechanism that focuses the search to the most
